...
export function apply(ctx) {
    ...
    const ui = { open: false, version: 0, currentSessionId: '', listeners: new Set() };
    const notify = () => { ui.version += 1; ui.listeners.forEach(listener => listener()); };
    const toggle = () => { ui.open = !ui.open; notify(); };
    const useUi = () => {
        const [, setVersion] = useState(ui.version);
        useEffect(() => {
            const listener = () => { setVersion(ui.version); };
...
const IntercomPanel = (props) => {
    ...
    const [groupRelays, setGroupRelays] = useState([]);
    ...
    const refreshLists = async () => {
        try {
            ...
            if (list.ok && list.value !== undefined) {
                ...
                if (convId === '' && first !== undefined)
                    setConvId(first.id);
            }
            const dormantList = await remote.dormant();
            if (dormantList.ok && dormantList.value !== undefined)
                setDormant(dormantList.value.conversations);
            ...
            if (groupList.ok && groupList.value !== undefined) {
                ...
                if (groupId === '' && firstGroup !== undefined)
            ...
            if (tab === 'conv') {
                ...
                if (isDormant) {
                    ...
                    if (r.ok && r.value !== undefined && r.value.ok) {
                        ...
                        say(`唤醒发送失败: ${r.value?.error ?? r.error?.message ?? 'unknown'}`, 'err');
                }
                else {
                    const r = await remote.send({ from: store.currentSessionId, targetId: convId, text, delivery });
                    if (r.ok && r.value !== undefined && r.value.ok)
                        say(`已发送 (${r.value.applied})`, 'ok');
                    else
                        say(`发送失败: ${r.value?.error ?? r.error?.message ?? 'unknown'}`, 'err');
                }
            }
            else {
                if (groupId === '') {
                ...
                if (r.ok && r.value !== undefined && r.value.ok) {
                    ...
                    say(`广播失败: ${r.value?.error ?? r.error?.message ?? 'unknown'}`, 'err');
            }
            setText('');
            void refreshHistory();
        }
    ...
    const createGroup = async () => {
        ...
        try {
            ...
            if (r.ok && r.value !== undefined && r.value.ok) {
                ...
                say(`建群失败: ${r.value?.error ?? r.error?.message ?? 'unknown'}`, 'err');
        }
        catch (error) {
    ...
    const addMember = async () => {
        ...
        try {
            ...
            if (r.ok && r.value !== undefined && r.value.ok) {
                ...
                say(`添加失败: ${r.value?.error ?? r.error?.message ?? 'unknown'}`, 'err');
        }
        catch (error) {
    ...
    const deleteGroup = async () => {
        ...
        try {
            ...
            if (r.ok && r.value !== undefined && r.value.ok) {
                ...
                say(`删除失败: ${r.value?.error ?? r.error?.message ?? 'unknown'}`, 'err');
        }
        catch (error) {
    ...
    return h('div', { className: 'dsh-ic-panel dsh-ic-wide', style: pos === null ? undefined : { left: pos.x, top: pos.y } }, h('div', { className: 'dsh-ic-head', onMouseDown: onHeadMouseDown }, h('span', { className: 'dsh-ic-head-title', dangerouslySetInnerHTML: { __html: ICON + 'Intercom 通信中心' } }), h('button', { className: 'dsh-ic-btn', onClick: startNew, title: '新建会话' }, '＋ 新会话'), h('button', { className: 'dsh-ic-btn', onClick: () => { void forkCurrent(); }, disabled: store.currentSessionId === '', title: 'Fork 当前会话' }, 'Fork'), h('button', { className: 'dsh-ic-iconbtn', title: '关闭', onClick: () => { store.open = false; notify(); }, dangerouslySetInnerHTML: { __html: CLOSE_ICON } })), h('div', { className: 'dsh-ic-body' }, h('div', { className: 'dsh-ic-side' }, h('div', { className: 'dsh-ic-tabs' }, h('button', { className: 'dsh-ic-tab' + (tab === 'conv' ? ' active' : ''), onClick: () => setTab('conv') }, '会话'), h('button', { className: 'dsh-ic-tab' + (tab === 'group' ? ' active' : ''), onClick: () => setTab('group') }, '群聊')), tab === 'conv'
        ...
        }, confirmDeleteGroup ? '⚠ 确认删除?再点一次执行' : '🗑 删除本群') : null)), h('div', { className: 'dsh-ic-main' }, h('div', { className: 'dsh-ic-chat-head' }, h('span', { className: 'dsh-ic-chat-title', title: currentTitle }, currentTitle), tab === 'conv' && currentConv !== undefined ? h('span', { className: 'dsh-ic-badge ' + (currentConv.status === 'running' ? 'running' : 'idle') }, currentConv.status === 'running' ? '忙碌' : '空闲') : null, tab === 'conv' && currentConv === undefined && currentDormant !== undefined ? h('span', { className: 'dsh-ic-badge idle' }, '💤 休眠 · 发送即唤醒') : null, tab === 'conv' && convId !== '' ? h('button', { className: 'dsh-ic-btn', onClick: () => openSession(convId), title: '在界面中打开' }, '打开') : null), tab === 'group' && groupId !== '' && groupRelays.length > 0 ? h('div', { className: 'dsh-ic-relays' }, h('div', { className: 'dsh-ic-relays-title' }, `沟通动态 (${groupRelays.length})`), groupRelays.map(r => h('div', { key: r.id, className: 'dsh-ic-relay' }, h('div', { className: 'dsh-ic-relay-head' }, r.toId === '*' ? `📢 ${r.fromTitle} → 全体成员` : `📤 ${r.fromTitle} → ${r.toTitle}`), h('div', { className: 'dsh-ic-relay-text' }, r.text)))) : null, (tab === 'conv' && convId === '') || (tab === 'group' && groupId === '')
        ? h('div', { className: 'dsh-ic-empty' }, '从左侧选择一个会话或群,开始查看和发送消息')
        : h('div', { className: 'dsh-ic-msgs' }, displayEntries.map((m, index) => h('div', { key: `${m.time}-${index}`, className: 'dsh-ic-msg ' + m.role }, m.memberTitle !== undefined && m.memberTitle !== '' ? h('span', { className: 'dsh-ic-msg-from' }, tab === 'group' && m.role === 'user' ? `${m.memberTitle} 的输入` : m.memberTitle) : null, m.text)), displayEntries.length === 0 ? h('div', { className: 'dsh-ic-empty' }, loadedOnce ? '暂无消息' : '加载中…') : null), h('div', { className: 'dsh-ic-composer' }, h('select', { className: 'dsh-ic-input', style: { width: 108, flex: 'none', margin: 0 }, value: delivery, onChange: (e) => setDelivery(e.target.value) }, h('option', { value: 'wake' }, '唤醒'), h('option', { value: 'steer' }, '介入')), h('textarea', { className: 'dsh-ic-input', style: { flex: 1, margin: 0, minHeight: 34, maxHeight: 90 }, rows: 1, value: text, onChange: (e) => setText(e.target.value), placeholder: tab === 'conv' ? (currentConv === undefined && currentDormant !== undefined ? '该会话休眠中,发送将唤醒它…' : '发消息给该会话…') : '广播给群成员…' }), h('button', { className: 'dsh-ic-btn dsh-ic-btn-primary', style: { flex: 'none' }, onClick: () => { void send(); } }, tab === 'conv' && currentConv === undefined && currentDormant !== undefined ? '唤醒并发送' : '发送')), h('div', { className: 'dsh-ic-feedback' + (feedback.tone === 'err' ? ' err' : feedback.tone === 'ok' ? ' ok' : ''), style: { flex: 'none', padding: '2px 12px 8px' } }, feedback.text))));
};
