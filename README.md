...
| **对话间消息** | 空闲目标**唤醒即开工**,忙碌目标排队不打断;介入(steer)需显式选择;限频(10 条/分钟/目标)+ 唤醒预算(3 次/人工输入重置)防消息风暴;来源定型包装防提示注入 |
...
- **Host Remote**:`@deepseek-ai/dsh-host-intercom` 以 Typert Remote 服务(`ctx.remote.intercom`)暴露 9 个方法(list/groups/send/broadcast/readConversation/readGroup/createGroup/addMember/removeMember),客户端经 `api-remotes` 挂载调用;同名服务类在宿主侧注册 11 个 `intercom_*` 模型工具。
- **投递**:空闲目标 `agent.followup`(唤醒开工),忙碌目标 `agent.inject`(排队),`steer` 显式介入;消息来源 `kind:'plugin' + form:'relay' + senderSessionId` 并包装「来自会话X」提示。
